...
#define MAX_MDNS_NAME_LEN 256 // The mDNS spec says this should never be more
                              // than 256 (including trailing '\0').

using namespace ns3;

// A single mDNS Query.
typedef struct Query {
  unsigned int buffer_pointer;                  // Position of Answer in packet.
                                                // (Used for debugging only.)
  char         qname_buffer[MAX_MDNS_NAME_LEN]; // Question Name: Contains the
                                                // object, domain or zone name.
  unsigned int qtype;                           // Question Type: Type of
                                                // question being asked by
                                                // client.
  unsigned int qclass;                          // Question Class: Normally the
                                                // value 1 for Internet (“IN”)
  bool         unicast_response;                //
  bool         valid;                           // False if problems were
                                                // encountered decoding packet.
} Query;

// A single mDNS Answer.
typedef struct Answer {
  unsigned int      buffer_pointer;                  // Position of Answer in
                                                     // packet. (Used for
                                                     // debugging only.)
  char              name_buffer[MAX_MDNS_NAME_LEN];  // object, domain or zone
                                                     // name.
  char              rdata_buffer[MAX_MDNS_NAME_LEN]; // The data portion of the
                                                     // resource record.
  unsigned int      rrtype;                          // ResourceRecord Type.
  unsigned int      rrclass;                         // ResourceRecord Class:
                                                     // Normally the value 1 for
                                                     // Internet (“IN”)
  unsigned long int rrttl;                           // ResourceRecord Time To
                                                     // Live: Number of seconds
                                                     // ths should be
                                                     // remembered.
  bool              rrset;                           // Flush cache of records
                                                     // matching this name.
  bool              valid;                           // False if problems were
                                                     // encountered decoding
                                                     // packet.
} Answer;
...
public:
  ...
  unsigned int buffer_pointer;          // Position in data_buffer while
                                        // processing packet.
  uint8_t data_buffer[MAX_PACKET_SIZE]; // Buffer containing mDNS packet.
  // Call this regularly to check for an incoming packet.
  int recvdns(Ptr<Packet>dnspacket,
              Address    from);

  // Send this MDns packet.
  void Send(Ptr<Socket>                       sockt,
            Ipv4Address                       addrs,
            TracedCallback<Ptr<const Packet> >tracer) const;
  ...
  MDns(Ptr<Socket>                       a,
       TracedCallback<Ptr<const Packet> >b);
...
private:
  ...
  bool         writeToBuffer(const uint8_t value,
                             char         *p_name_buffer,
                             int          *p_name_buffer_pos,
                             const int     name_buffer_len);
  int nameFromDnsPointer(char          *p_name_buffer,
                         int            name_buffer_pos,
                         const int      name_buffer_len,
                         const uint8_t *p_packet_buffer,
  ...
  int nameFromDnsPointer(char          *p_name_buffer,
                         ...
                         const uint8_t *p_packet_buffer,
                         int            packet_buffer_pos);
  int parseText(char          *data_buffer,
                const int      data_buffer_len,
                const int      data_len,
                const uint8_t *p_packet_buffer,
                int            packet_buffer_pos);
